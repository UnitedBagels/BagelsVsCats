...
class Cat(pygame.sprite.Sprite):
	...
	def update(self, paused):
		...
		if self.paused == False:
			if self.move == True:
				if self.catType != "alien_cat":
					self.rect.x -= self.speed
				else:
					self.moveTimer -= 1

				if 0 < self.moveTimer <= 40:
					self.rect.x -= 1
					self.moveTimer -= 1
				if self.moveTimer <= 0:
					self.moveTimer = 120
	...
	def alienFire(self,bagelList,bulletImage,alienBulletList,allSprites):
		maxX = -1
		target = None
		for i in bagelList:
			if (i.rect.y <= self.rect.y <= i.rect.y + 23 or i.rect.y <= self.rect.y + 23 <= i.rect.y + 23) and (i.rect.x < self.rect.x) and (i.rect.x + self.rect.x >= 0) and self.eat == False and i.bagelType != "flagel": # Ignore throwing melons at flagels
		...
		if self.fired == True and self.eat == False and self.paused == False:
			self.fireTimer += 1

		if self.fireTimer >= 1000:
			bullet = AlienBullet(self.rect.x + 14, self.rect.y - 10,bulletImage, self.shoot)
			bullet.add(alienBulletList)
			bullet.add(allSprites)
			self.fireTimer = 0
